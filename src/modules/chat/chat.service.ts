...
export class ChatService {
  ...
  async getChatChannelsByUser(userId: number) {
    const result: {
      ...
      'SELECT c.id chatId, message, c.created_at createdAt, user_id userId, driver_id driverId, is_driver isDriver, d.*  FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY driver_id ORDER BY created_at asc) as row_num FROM chats WHERE user_id = ?) AS c inner join drivers d on c.driver_id = d.id WHERE row_num = 1 order by c.created_at desc',
      [userId],
    );
    return result.map((r) => {
      const {
  ...
  async getChatChannelsByDriver(driverId: number) {
    const result: {
      ...
      'SELECT c.id chatId, message, c.created_at createdAt, user_id userId, driver_id driverId, is_driver isDriver, d.*  FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY driver_id ORDER BY created_at asc) as row_num FROM chats WHERE driver_id = ?) AS c inner join users u on c.user_id = u.id WHERE row_num = 1 order by c.created_at desc',
      [driverId],
    );
    return result.map((r) => {
      const {
