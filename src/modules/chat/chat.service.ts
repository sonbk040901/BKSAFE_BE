...
export class ChatService {
  ...
  async getChatChannelsByDriver(driverId: number) {
    const result: {
      ...
      'SELECT c.id chatId, message, c.created_at createdAt, user_id userId, driver_id driverId, is_driver isDriver, d.*  FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY driver_id ORDER BY created_at DESC) as row_num FROM chats WHERE driver_id = ?) AS c inner join users u on c.user_id = u.id WHERE row_num = 1 order by c.created_at desc',
      [driverId],
    );
    return result.map((r) => {
      const {
